...
class AdaMod(Optimizer):
    ...
    Arguments:
        ...
        betas (Tuple[float, float, float], optional): coefficients used for computing
            running averages of gradient, its square, and smoothing coefficient for adaptive
            learning rates (default: (0.9, 0.999, 0.999))
        eps (float, optional): term added to the denominator to improve
            numerical stability (default: 1e-8)
        weight_decay (float, optional): weight decay (L2 penalty) (default: 0)
    """

    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999, 0.999), eps=1e-8, weight_decay=0):
        if not 0.0 <= lr:
        ...
        if not 0.0 <= betas[2] < 1.0:
            raise ValueError("Invalid beta3 parameter: {}".format(betas[2]))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                exp_avg_lr.mul_(beta3).add_(1 - beta3, step_size)
